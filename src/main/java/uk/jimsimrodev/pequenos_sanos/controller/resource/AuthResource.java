...
public interface AuthResource {
        ...
        @Operation(summary = "Registrar nuevo usuario", description = "Crea una cuenta de padre/tutor con email único y contraseña hasheada con BCrypt.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Usuario registrado exitosamente", content = @Content(schema = @Schema(implementation = DatosRespuestaUsuario.class))),
        ...
        @Operation(summary = "Iniciar sesión", description = "Autentica al usuario con email y contraseña, retorna un token JWT válido por 2 horas.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Autenticación exitosa, token JWT generado", content = @Content(schema = @Schema(implementation = DatosJWTToken.class))),
