...
public interface AlimentoResource {
    ...
    @Operation(
            summary = "Listar alimentos del catálogo",
            description = "Retorna todos los alimentos activos. Se puede filtrar opcionalmente por categoría."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de alimentos",
                    content = @Content(schema = @Schema(implementation = DatosRespuestaAlimento.class))),
            @ApiResponse(responseCode = "401", description = "Token ausente o inválido")
    })
    @GetMapping
    ResponseEntity<List<DatosRespuestaAlimento>> listar(
            @Parameter(description = "Filtrar por categoría (opcional)", example = "FRUTA")
            @RequestParam(required = false) CategoriaAlimento categoria);

    @Operation(
            summary = "Obtener alimento por ID",
            description = "Retorna el detalle de un alimento activo por su ID."
    )
    @ApiResponses(value = {
    ...
    ResponseEntity<DatosRespuestaAlimento> buscarPorId(
            @Parameter(in = ParameterIn.PATH, name = "id", description = "ID del alimento", example = "1")
