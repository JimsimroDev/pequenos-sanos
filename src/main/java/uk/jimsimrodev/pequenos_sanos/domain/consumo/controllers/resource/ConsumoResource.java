...
public interface ConsumoResource {
    ...
    @Operation(
            ...
            description = "Valida el alimento y perfil, persiste el registro y acredita la recompensa en una sola transacción."
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Consumo registrado y recompensa acreditada",
                    content = @Content(schema = @Schema(implementation = DatosRespuestaConsumo.class))),
    ...
    @Operation(
            summary = "Historial de consumo de un perfil",
            description = "Retorna todos los registros de consumo de un perfil, ordenados por más reciente primero."
    )
    @ApiResponses(value = {
    ...
    ResponseEntity<List<DatosRespuestaConsumo>> listarPorPerfil(
            @Parameter(in = ParameterIn.PATH, name = "perfilId", description = "ID del perfil infantil", example = "1")
