# Design: Realistic Market/Farm Map via MapRenderer

## Technical Approach

Replace 500+ individual tile rectangles + emoji decorations with 5 Phaser `Graphics` objects (one per depth layer) using only `fillRect`, `fillCircle`, `fillTriangle`, `lineBetween`. The map represents 4 themed zones in a 1200×960 world (25×20 tiles at 48px). `MapRenderer.draw(scene)` is the single entry point — one line replaces the current `drawTileMap()` + `createDecorations()`.

## Architecture Decisions

| Decision | Options | Choice | Rationale |
|----------|---------|--------|-----------|
| Module pattern | Class static methods vs. plain functions | **Static class** | Groups private helpers under one exported symbol; `MapRenderer.draw(scene)` matches the contract exactly |
| File count | Single file vs. split by zone | **Single `MapRenderer.ts`** | ~260 primitives all tightly coupled to same coordinate system; splitting adds import overhead with no reuse benefit |
| Zone coords | Relative offsets vs. absolute constants | **Named constants** | Every zone, building, path is a descriptive `const`; enables visual reasoning and easy tuning |
| Label rendering | Graphics text vs. Phaser.Text | **Phaser.Text** | Graphics has no text API; Text is a valid primitive (no asset dependency) |
| Draw order within layer | Back-to-front grouping | **Back-to-front** | Buildings before their foreground elements; barn body → door → roof |

## Data Flow

```
GameScene.create()
  └─ MapRenderer.draw(this)          ← single line replaces 2 methods
       ├─ g0 = scene.add.graphics()  → drawBaseGround()       → depth 0
       ├─ g1 = scene.add.graphics()  → drawTerrain()          → depth 1
       ├─ g2 = scene.add.graphics()  → drawBuildings()        → depth 2
       ├─ g3 = scene.add.graphics()  → drawVegetation()       → depth 3
       ├─ g4 = scene.add.graphics()  → drawFineDecorations()  → depth 4
       └─ drawLabels(scene)          → 4× Phaser.Text objects → depth 4
```
...
|------|--------|-------------|
| `frontend/src/game/map/MapRenderer.ts` | Create | Static class with 5 layer methods + 7 helpers + color/coord constants |
| `frontend/src/game/scenes/GameScene.ts` | Modify | Remove `drawTileMap`, `createDecorations`, `tileMap` field; replace with `MapRenderer.draw(this)` |

## Interfaces / Contracts

```typescript
// MapRenderer.ts — single public export
...
|-------|-------------|----------|
| Visual | Map renders all 4 zones correctly | Manual visual verification in browser |
| Integration | GameScene create() works with MapRenderer | Smoke test — no console errors |
| Unit | Helper functions | Difficult with Phaser Graphics API; integration-level verification preferred |
...
No migration required. One-line change in `GameScene.create()`. Rollback: restore deleted lines.
...
const FENCE_POST    = 0x8b7355
const FENCE_RAIL    = 0xa0895f
...
const MAP_COLS = 25
const MAP_ROWS = 20
const MAP_W    = 1200   // MAP_COLS * TILE
const MAP_H    = 960    // MAP_ROWS * TILE

const ZONE_CX  = 600    // center X
const ZONE_CY  = 480    // center Y
const PATH_W   = 28     // path width

// ── FARM (top-left: 0..600 × 0..480) ──
const BARN_X      = 250
const BARN_Y      = 200
const BARN_W      = 180
const BARN_H      = 110
...
const SILO_R      = 18
const PASTURE_X1  = 70
const PASTURE_Y1  = 320
const PASTURE_X2  = 330
const PASTURE_Y2  = 440
const POND_X      = 480
const POND_Y      = 380
const POND_R      = 42
const WINDMILL_X  = 130
const WINDMILL_Y  = 150

// ── ORCHARD (top-right: 600..1200 × 0..480) ──
const GH_X        = 900
const GH_Y        = 200
const GH_W        = 180
const GH_H        = 100
...
// ── MARKET (bottom-left: 0..600 × 480..960) ──
const STALL1_X    = 280
const STALL1_Y    = 700
const STALL2_X    = 120
const STALL2_Y    = 750
...
// ── PICNIC (bottom-right: 600..1200 × 480..960) ──
const TABLE1_X    = 760
const TABLE1_Y    = 620
const TABLE2_X    = 940
const TABLE2_Y    = 720
...
| Zone | Base | Terrain | Buildings | Vegetation | Decor | Total |
|------|------|---------|-----------|------------|-------|-------|
| Farm (top-left) | 10 | 4 | 24 | 14 | 10 | 62 |
| Orchard (top-right) | 8 | 6 | 16 | 40 | 8 | 78 |
| Market (bottom-left) | 4 | 2 | 20 | 6 | 10 | 42 |
| Picnic (bottom-right) | 4 | 2 | 18 | 16 | 12 | 52 |
| Center | — | 2 | — | — | — | 2 |
| Shared (paths, labels) | 4 | 3 | — | 6 | 8 | 21 |
| **Total** | **30** | **19** | **78** | **82** | **48** | **~257** |

## GameScene Integration

```typescript
// Remove these fields and methods from GameScene:
// - tileMap: Phaser.GameObjects.Rectangle[][]     (line 87)
// - drawTileMap(mapW, mapH)                        (lines 119-136)
// - createDecorations(mapW, mapH)                  (lines 138-157)
